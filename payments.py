...
def stripe_pay(amount, token, user):
    """
    ...
    try:
        charge = stripe.Charge.create(
            ...
            currency="usd",
            card=token,
            description="Site Notifier upgrade for user_id {}, email {}".format(user.user_id(), user.email())
        )
        logging.info('payment id: {}'.format(charge.id))
        return charge, "Payment successful"
    except stripe.CardError, e:
        return None, "The card was declined, please check card information and try again"
